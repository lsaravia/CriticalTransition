...
plotCritical_Clusters<-function(Clusters,time,metaNsp,dispDist,migr, k)
{
  ...
  #
  print(ggplot(mClusters, aes(x=ReplacementRate, y=SpanningProb)) + geom_point() + theme_bw() + xlim(0,0.02) + facet_grid(Side ~ MetaType ) +xlab(bquote(rho)) + geom_line(colour="red") + geom_vline(aes(xintercept=pcrit),k,colour="green"))
