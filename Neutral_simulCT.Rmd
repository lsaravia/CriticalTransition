...
## Time series simulations : Side 512, time=5000 metacommunity=L
...
ggplot(filter(rhoCrit,ColonizationRate==round(0.0001596079,9)),aes(x=DispersalDistance,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ theme_bw() 
...
k1$ColonizationRate <- m
rhoCrit <- rbind(rhoCrit,k1[,c(3,5,6,1,2)])
rhoCrit <- arrange(rhoCrit,MetaType,desc(DispersalDistance))
pandoc.table(rhoCrit)
...
## Simulations to Time=5000 with side=500, dispersal=26 migration=0.0001596079 species=256 

```{r simuldisp_clu_side512_meta256_n30_T5000, eval=T,echo=F,message=T,warning=T}
setwd("Simul")
