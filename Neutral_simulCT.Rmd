...
p <-expand.grid(disp=2.038974,migr=0.0001596079,repl=ReplRate,side=c(100,256,512),meta=c("L")) 
...
# Calculation of the critical point T=3000, have to re-do the simulations  
...
# Calculation of the critical point T=5000 DispersalDistance=26
...
require(plyr)
require(dplyr)
mClusters <- filter(Clusters, Time==5000,MetaNsp==64,DispersalDistance==alfa,ColonizationRate==m) %>% group_by(MetaNsp,Side,MetaType,ReplacementRate) %>% summarise(MaxClusterProp=median(MaxClusterProp),n=n(),SpanningProb=sum(ifelse(SpanningSpecies>0,1,0))/n,SpanningClust=mean(SpanningClust))
...
require(pander)
pandoc.table(k,round=5)
rhoCritSide <- rbind(rhoCritSide,k[,c(1,5,6,2:4)])
#
# finite size scaling to determine pcrit for infinite lattices
#
k1 <- group_by(k,MetaType) %>% mutate(iSide=1/(Side*Side) ) %>% do(model=lm(pcrit ~ iSide, data=.)) %>% summarise(pcrit=predict(model,newdata=data.frame(iSide=0)),pcSE=predict(model,newdata=data.frame(iSide=0),se.fit=T)$se.fit)                                                 
...
k1$MetaType <- c("L","U") 
k1$iSide <- 0 
k1$DispersalDistance<-26.67
k1$ColonizationRate <- m
rhoCrit <- k1[,c(3,5,6,1,2)]
pandoc.table(rhoCrit)
...
tClusters <- Clusters %>% filter(Time==time,MetaNsp==nsp,DispersalDistance==alfa,ColonizationRate==m)  %>% mutate(MetaType=factor(MetaType,labels=c("Logseries","Uniform")))
k$MetaType <- factor(k$MetaType,labels=c("Logseries","Uniform"))
...
# Calculation of the critical point T=5000 migration=0.00016 dispersal=13
...
k <- k %>% summarise(pcrit=approx(SpanningProb,ReplacementRate,xout=0.5)[["y"]],critClust=approx(SpanningProb,SpanningClust,xout=0.5)[["y"]])
k$DispersalDistance<-round(mean_power(alfa),2) 
k$ColonizationRate <- m
require(pander)
pandoc.table(k,round=5)
rhoCritSide <- rbind(rhoCritSide,k[,c(1,5,6,2:4)])
...
k1$MetaType <- c("L","U") 
k1$iSide <- 0 
k1$DispersalDistance<-round(mean_power(alfa),2) #13.34
k1$ColonizationRate <- m
rhoCrit <- rbind(rhoCrit,k1[,c(3,5,6,1,2)])
...
plotCritical_Clusters(Clusters,5000,nsp,alfa,m)
...
# Plot Critical Clusters H and Richness vs Replacement rate for migration=0.0016 dispersal=26.66
...
mClusters <- filter(Clusters, Time==5000,MetaNsp==nsp,DispersalDistance==alfa,ColonizationRate==m) %>% group_by(MetaNsp,Side,MetaType,ReplacementRate,DispersalDistance) %>% summarise(MaxClusterProp=median(MaxClusterProp),n=n(),SpanningProb=sum(ifelse(SpanningSpecies>0,1,0))/n,SpanningClust=mean(SpanningClust))
...
k1$MetaType <- c("L","U") 
k1$iSide <- 0 
k1$DispersalDistance<-round(mean_power(alfa),2) # 26.67 
k1$ColonizationRate <- m
rhoCrit <- rbind(rhoCrit,k1[,c(3,5,6,1,2)])
rhoCrit <- arrange(rhoCrit,MetaType)
pandoc.table(rhoCrit)
save.image()
...
# Plot critical point for m=0.016 dispersal=26.67
...
# Calculates the probability of spanning cluster
#
k <- group_by(mClusters, MetaType,Side) %>% filter(ReplacementRate>=0.01 & ReplacementRate<=0.015)
# Delete one record
#k <- k[-6,]
...
ggplot(filter(rhoCrit,DispersalDistance!=13.34),aes(x=ColonizationRate,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ scale_x_log10() + theme_bw() 

ggplot(filter(rhoCrit,ColonizationRate==0.0001596079),aes(x=DispersalDistance,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ theme_bw() 
...
# Plot critical point for m=0.00016 dispersal=6.67
...
mClusters <- filter(Clusters, Time==time,MetaNsp==nsp,DispersalDistance==alfa,ColonizationRate==m) %>% group_by(MetaNsp,Side,MetaType,ReplacementRate,DispersalDistance) %>% summarise(MaxClusterProp=median(MaxClusterProp),n=n(),SpanningProb=sum(ifelse(SpanningSpecies>0,1,0))/n,SpanningClust=mean(SpanningClust))
...
# Calculates the probability of spanning cluster
#
k <- group_by(mClusters, MetaType,Side) %>% filter(SpanningProb>=0.30 & SpanningProb<=0.70)
# Delete one record
#k <- k[-6,]
...
ggplot(filter(rhoCrit,round(DispersalDistance,1)==26.7),aes(x=ColonizationRate,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ scale_x_log10() + theme_bw() + xlab("Migration") + ylab("Critical Probability")
ggsave("figs/CritProb_Migr_T5000_64_meta.png", width=6,height=4,units="in",dpi=600)

#rhoCrit <- mutate(rhoCrit,ColonizationRate=ifelse(ColonizationRate==0.0001596079,0.000159608,ColonizationRate))

ggplot(filter(rhoCrit,round(ColonizationRate,9)==0.000159608),aes(x=DispersalDistance,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ theme_bw() +xlab("Dispersal Distance") + ylab("Critical Probability")
ggsave("figs/CritProb_Disp_T5000_64_meta.png", width=6,height=4,units="in",dpi=600)
...
```{r plotFitdisp26_cluAll_side_64_meta30_T5000, eval=T,echo=F,message=T,warning=T}
source("R/powerlaw/discpowerexp.R")
source("R/powerlaw/discexp.R")
source("R/powerlaw/powerexp.R")
source("R/powerlaw/exp.R")
...
namType <- c("a) Spanning","b) Abundant","d) Other Abund","c) Other Spanning")
...
ggplot(mMdl, aes(x=model,y=Frequency,fill=type)) + geom_bar(stat="identity") +  scale_fill_manual(values=mc) + theme_bw()


# Select Power or Power exponential models
#
mMdl <- filter(Models,DeltaAIC==0,MetaType=="L",model %in% c("PowExp","Pow")) %>% mutate(typeAb=namTypeAb[type],type=namType[type],rate=ifelse(model=="PowExp",rate,NA))

# Plot of power exponent 
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line") + 
  scale_x_continuous(breaks=c(0,0.0010,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) +guides(colour=F) + ylab("Power exponent")+ xlab(bquote(rho))
ggsave("figs/discExp_T5000_64_256_Logser.png", width=6,height=4,units="in",dpi=600)


ggplot(mMdl,aes(x=ReplacementRate,y=rate,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line")  + 
  scale_x_continuous(breaks=c(0,0.0010,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) +guides(colour=F) + ylab("Exponential rate") + xlab(bquote(rho))
ggsave("figs/discRate_T5000_64_256_Logser.png", width=6,height=4,units="in",dpi=600)


# Select Power or Power exponential models for Metacommunity  Uniform
#
mMdl <- filter(Models,DeltaAIC==0,MetaType=="U",model %in% c("PowExp","Pow")) %>% mutate(typeAb=namTypeAb[type],type=namType[type],rate=ifelse(model=="PowExp",rate,NA))

ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line") +
  scale_x_continuous(breaks=c(0,0.0010,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) +guides(colour=F)+ ylab("Power exponent")+ xlab(bquote(rho))
ggsave("figs/discExp_T5000_64_256_Unif.png", width=6,height=4,units="in",dpi=600)
...
ggsave("figs/discRate_T5000_64_256_Unif.png", width=6,height=4,units="in",dpi=600)

# Exponential rate of other sp
#
mMdl <- filter(Models,model=="PowExp",type==3 | type==4) %>% mutate(typeAb=namTypeAb[type],type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=rate,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red") + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Exponential rate") + xlab(bquote(rho))
ggsave("figs/discRateOther_T5000_64_256_meta.png", width=6,height=4,units="in",dpi=600)

# Power exponent rate of other sp
#
mMdl <- filter(Models,model=="PowExp",type==3 | type==4) %>% mutate(typeAb=namTypeAb[type],type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red") + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Power exponent") + xlab(bquote(rho))
ggsave("figs/discExpOther_T5000_64_256_meta.png", width=6,height=4,units="in",dpi=600)

# Power exponent rate of spanning or most abundant  sp
#
mMdl <- filter(Models,model=="PowExp",type==1 | type==2) %>% mutate(typeAb=namTypeAb[type],type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red") + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Power exponent") + xlab(bquote(rho))
ggsave("figs/discExpSpan_T5000_64_256_meta.png", width=6,height=4,units="in",dpi=600)

# Exponential rate of spanning or  most abundant sp
#
mMdl <- filter(Models,model=="PowExp",type==1 | type==2) %>% mutate(typeAb=namTypeAb[type],type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=rate,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red") + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Exponential rate") + xlab(bquote(rho))
ggsave("figs/discRateSpan_T5000_64_256_meta.png", width=6,height=4,units="in",dpi=600)
