...
# Juntar las 3 figuras en 1 en el pdf ************************************
#

# multiplot(g1,g2,g3,layout=matrix(c(1,2,3,4),nrow=2,byrow=T))
#
# library(gtable)
# library(gridExtra)
# 
...
# grid.newpage()
# grid.arrange(gA, gC,gB, nrow = 2)
# 
# ggsave("figs/CritProb_Migr_Disp_Mnsp_T5000.png", width=6,height=6,units="in",dpi=600)
...
nMdl <-group_by(Models,MortalityRate,DispersalDistance,ColonizationRate,ReplacementRate,MetaType,Rep) %>% filter(DeltaAIC==0)%>% summarize(n=n())

Models1<-group_by(Models,MortalityRate,DispersalDistance,ColonizationRate,ReplacementRate,MetaType,Rep,model) %>% summarize(n=n()) %>% filter(n>2)  %>% inner_join(Models) %>% arrange(type)
# inside group we can have 1 and 4 or 2 and 3,this means n=2 or n=4, if there is a shift from most abundant to spanning !!! n=3 is wrong!!!!

Models1 <-group_by(Models,MortalityRate,DispersalDistance,ColonizationRate,ReplacementRate,Rep,type,Species) %>% arrange(DeltaAIC) %>% slice(2) %>% filter(DeltaAIC<2)

namType <- c("Spanning","Abundant","Other Abund","Other Spanning")
...
ggplot(mMdl, aes(x=model,y=Frequency,fill=type)) + geom_bar(stat="identity") +  scale_fill_manual(values=mc,name="Species",breaks=c("Spanning","Abundant","Other Spanning","Other Abund")) + scale_x_discrete(labels=c("No Model","Power","Power Exp.")) + theme_bw()
...
ggplot(mMdl,aes(x=ReplacementRate,y=rate,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=median,geom="line",colour=mc[3]) + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Exponential rate") + xlab(bquote(rho))+ scale_colour_manual(values=mc)
ggsave("figs/discRateOther_T5000_64_256_meta.png", width=6,height=4,units="in",dpi=600)

# Power exponent rate of other sp
#
mMdl <- filter(Models,model=="PowExp",type==3 | type==4) %>% mutate(typeAb=namTypeAb[type],type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=median,geom="line",colour=mc[3]) + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Power exponent") + xlab(bquote(rho))+ scale_colour_manual(values=mc)
...
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour=mc[3]) + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Power exponent") + xlab(bquote(rho)) + scale_colour_manual(values=mc)
...
#
mMdl <- filter(Models,model=="PowExp",type==1 | type==2) %>% mutate(typeAb=namTypeAb[type],type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=rate,colour=type)) + geom_point() +  theme_bw() + facet_wrap(~MetaType,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour=mc[3]) + scale_x_continuous(breaks=c(0,0.0010,0.0020,0.0030,0.005,0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab("Exponential rate") + xlab(bquote(rho))+ scale_colour_manual(values=mc)
...
simul_NeutralSpatPatt(nsp,side,alfa,m,ReplRate,"S",time,100,"L",T)
