...
## Simulations to calculate the clusters sizes, side=100 recording only the last time=3000


```{r gensimul_clu_100_64_U30, eval=F,echo=F,message=T,warning=T}
setwd("Simul")
...
Clu <- foreach(i=1:length(ReplRate),.combine='rbind') %dopar%
{
    simulNeutral_1Time(nsp,side,alfa,m,ReplRate[i],"S",time,nSimul)
}
stopCluster(cl)
rm(cn,cl)
...
td1 <-calc_CommunityDist_1T(nsp,side,alfa,m,ReplRate,time,nSimul)
td1 <- select(td1, -X1, -ReplacementRate) %>% rename(Repl=ReplacementRate.1) %>% mutate(MetaNsp=nsp,Side=side,Disp=alfa,Migr=m) 
tdist1 <- rbind(tdist1,td1[,c(6:9,1:5)])
...
# Simulations to calculate the clusters sizes, side=256 ,time=3000

```{r gensimul_clu_256_64_U30, eval=F,echo=F,message=T,warning=T}
setwd("Simul")
...
Clu <- foreach(i=1:length(ReplRate),.combine='rbind') %dopar%
{
    simulNeutral_1Time(nsp,side,alfa,m,ReplRate[i],"S",time,nSimul,F)
}
stopCluster(cl)
rm(cn,cl)
Clusters <-rbind(Clusters,Clu)

# Calculate KL distances and Bray-Curtis for only 1 time 
...
## Simulations to calculate the clusters sizes, side=512 ,time=3000 metacommunity=Uniform

```{r gensimul_clu_512_64_U30, eval=F,echo=F,message=T,warning=T}
setwd("Simul")
...
Clu <- foreach(i=1:length(ReplRate),.combine='rbind') %do%
{
    simulNeutral_1Time(nsp,side,alfa,m,ReplRate[i],"S",time,nSimul,F)
}
stopCluster(cl)
...
td1 <-calc_CommunityDist_1T(nsp,side,alfa,m,ReplRate,time,nSimul,"U")
td1 <- select(td1, -X1, -ReplacementRate) %>% rename(Repl=ReplacementRate.1) %>% mutate(MetaNsp=nsp,Side=side,Disp=alfa,Migr=m,MetaType="U") 
tdist1 <- rbind(tdist1,td1[,c(6:10,1:5)])
...
## Time series simulations to Time = 3000 for logseries side=100 species=64

```{r plotsimul100_64_L5, eval=F,error=F,echo=F,message=T,warning=T}
setwd("Simul")
...
CT1 <- foreach(i=1:length(ReplRate),.combine='rbind') %dopar%
{
  simul_NeutralPlotTime(nsp,side,alfa,m,ReplRate[i],T,3000,nSimul,"N","L")
}
stopCluster(cl)

rm(cn,cl)
for(i in 1:length(ReplRate)){
  kk <- simul_NeutralPlotTime(nsp,side,alfa,m,ReplRate[i],F,3000,nSimul,"N","L"))
}
...
## Simulations Logseries side =256 species=64 1T=3000

```{r gensimul_clu_256_64_L30, eval=F,echo=F,message=T,warning=T}
setwd("Simul")
...
Clu <- foreach(i=1:length(ReplRate),.combine='rbind') %dopar%
{
    simulNeutral_1Time(nsp,side,alfa,m,ReplRate[i],"S",time,nSimul,T,"N","L")
}
stopCluster(cl)
rm(cn,cl)
Clusters <-rbind(Clusters,Clu)
...
td1 <-calc_CommunityDist_1T(nsp,side,alfa,m,ReplRate,time,nSimul,"L")
td1 <- select(td1, -X1, -ReplacementRate) %>% rename(Repl=ReplacementRate.1) %>% mutate(MetaNsp=nsp,Side=side,Disp=alfa,Migr=m,MetaType="L") 
tdist1 <- rbind(tdist1,td1[,c(6:10,1:5)])
...
## Simulations Logseries side =512 species=64 1T=3000

```{r gensimul_clu_512_64_L30, eval=F,echo=F,message=T,warning=T}
setwd("Simul")
...
## Simulations Logseries side =100 species=64 1T=3000

```{r gensimul_clu_100_64_L30, eval=F,echo=F,message=T,warning=T}
setwd("Simul")
...
## Plot simulations made to T=3000, 

```{r plot_1T_simul_U30, eval=F,echo=F,message=T,warning=T}

require(ggplot2)

ggplot(tdist1, aes(x=Repl, y=bray)) + geom_point() + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red") + facet_grid(Side ~ MetaType )

ggplot(tdist1, aes(x=Repl, y=KL)) + geom_point() + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red") + facet_grid(Side ~ MetaType )

ggplot(tdist1, aes(x=Repl, y=ks)) + geom_point() + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red") + facet_grid(Side ~ MetaType )

ggplot(Clusters, aes(x=ReplacementRate, y=MaxClusterProp)) + geom_point() + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red") + facet_grid(Side ~ MetaType )

ggplot(Clusters, aes(x=ReplacementRate, y=H)) + geom_point(alpha=.2) + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red")+ facet_grid(Side ~ MetaType )+xlab(bquote(rho))
ggsave("figs/HvsRepl_T3000_64_side_meta.png", width=6,height=6,units="in",dpi=600)


ggplot(Clusters, aes(x=ReplacementRate, y=Richness)) + geom_point(alpha=.2) + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red")+ facet_grid(Side ~ MetaType,scales="free_y") +xlab(bquote(rho))
ggsave("figs/RichvsRepl_T3000_64_side_meta.png", width=6,height=6,units="in",dpi=600)


ggplot(Clusters, aes(x=ReplacementRate, y=TotalClusters/(Side*Side))) + geom_point() + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red")+ facet_grid(Side ~ MetaType )

ggplot(Clusters, aes(x=ReplacementRate, y=SpanningClust)) + geom_point(alpha=.2) + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red")+ facet_grid(Side ~ MetaType ) + ylab("Spanning Cluster Size") + xlab(bquote(rho))
ggsave("figs/SpanCluvsRepl_T3000_64_side_meta.png", width=6,height=6,units="in",dpi=600)

require(plyr)
require(dplyr)
mClusters <- group_by(Clusters,MetaNsp,Side,MetaType,ReplacementRate) %>% summarise(MaxClusterProp=median(MaxClusterProp),n=n(),SpanningProb=sum(ifelse(SpanningSpecies>0,1,0))/n,SpanningClust=median(SpanningClust))

ggplot(mClusters, aes(x=ReplacementRate, y=SpanningProb)) + geom_point() + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red")+ facet_grid(Side ~ MetaType )
...
setwd(oldcd)
ggsave("figs/SAD_T3000_64_100U.png", width=6,height=6,units="in",dpi=600)
...
setwd(oldcd)
ggsave("figs/SAD_T3000_64_256U.png", width=6,height=6,units="in",dpi=600)
...
plotNeutral_SAD_1T(64,512,ReplRate,time,"L")
...
#ReplRate <- c(0,0.1)
ReplRate <- c(0.0005,0.0014,0.0018,0.0022,0.0026,0.0028)
#time <- 100
time <- 3000
...
#
# replace rho=0 by a very low vallue to use stat_smooth(se=F)
#

k <-group_by(mClusters, Side,MetaType) %>% filter(SpanningProb>0.29 & SpanningProb<0.8) %>% summarise(pcrit=approx(SpanningProb,ReplacementRate,xout=0.5)$y,critClust=approx(SpanningProb,SpanningClust,xout=0.5)$y)
...
ggplot(Clusters, aes(x=ReplacementRate, y=H)) + geom_point(alpha=.2) + theme_bw() + scale_x_log10(breaks=c(0.003,0.02,0.1,1)) + stat_summary(fun.y=median,geom="line",colour="red")+ facet_grid(Side ~ MetaType )+xlab(bquote(rho)) + geom_vline(aes(xintercept=pcrit),k,colour="green")
