...
require(plyr)
require(dplyr)
mClusters <- filter(Clusters, Time==5000,MetaNsp==64,DispersalDistance==2.03897,ColonizationRate==m) %>% group_by(MetaNsp,Side,MetaType,ReplacementRate) %>% summarise(MaxClusterProp=median(MaxClusterProp),n=n(),SpanningProb=sum(ifelse(SpanningSpecies>0,1,0))/n,SpanningClust=mean(SpanningClust))
...
ggplot(filter(rhoCrit,round(ColonizationRate,9)==0.000159608),aes(x=DispersalDistance,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ theme_bw() 
...
# Plot the cluster sizes with fitted functions, and relation of power exponent with ReplacementRate
 
```{r plotdisp26_cluAll_side_64_meta30_T5000, eval=T,echo=F,message=T,warning=T}
setwd(oldcd)
source("R/powerlaw/discpowerexp.R")
source("R/powerlaw/discexp.R")
source("R/powerlaw/powerexp.R")
...
namType <- c("c) Spanning","a) Abundant","b) Other Abund","d) Other Spanning")
mMdl <- filter(Models,model=="PowExp",MetaType=="L") %>% mutate(type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=alfa)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red")
ggplot(mMdl,aes(x=ReplacementRate,y=rate)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red") 

mMdl <- filter(Models,model=="PowExp",MetaType=="U") %>% mutate(type=namType[type])
ggplot(mMdl,aes(x=ReplacementRate,y=alfa)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red")
ggplot(mMdl,aes(x=ReplacementRate,y=rate)) + geom_point() +  theme_bw() + facet_wrap(~type,scales="free_y") + stat_summary(fun.y=mean,geom="line",colour="red") 
