...
require(ggplot2)
ggplot(filter(rhoCrit,round(DispersalDistance,1)==26.7),aes(x=ColonizationRate,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=0)+ scale_x_log10() + theme_bw() + xlab("Migration") + ylab(bquote(rho[c]))
...
ggplot(filter(rhoCrit,round(ColonizationRate,9)==0.000159608,MetaNsp==64),aes(x=DispersalDistance,y=pcrit,colour=MetaType)) + geom_point() + geom_errorbar(aes(ymin=pcrit-pcSE, ymax=pcrit+pcSE), width=.1)+ theme_bw() +xlab("Dispersal Distance") + ylab(bquote(rho[c]))
...
```{r simuldisp_clu_side512_meta16_n30_T5000, eval=T,echo=F,message=T,warning=T}
setwd("Simul")


side <- 512 #c(100,150,256,512)
nsp <- 16
nSimul <- 30
...
p <-expand.grid(disp=alfa,migr=m,repl=ReplRate,side=side,meta=c("L")) # ,"L"
...
pandoc.table(select(rhoCrit,c(6,1:5)) %>% arrange(MetaNsp,MetaType,DispersalDistance,ColonizationRate)
             ,round=5)
