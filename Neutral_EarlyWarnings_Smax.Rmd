...
p <-expand.grid(disp=alfa,migr=m,repl=ReplRate,side=side,meta=c("L","U"),rndI=rndI)
time <-rep(time, times=nrow(p)/length(time))
p <-cbind(p,time)
...
TS1 <- foreach(i=1:length(ReplRate),.combine='rbind') %dopar%
{
    ...
    #simul_NeutralTimeSeries(nsp,p$side[i],p$disp[i],p$migr[i],p$repl[i],FALSE,p$time[i],nSimul,"N",p$meta[i],"S",2,p$timeI[i],timeDelta,p$rndI[i],TRUE)
...
# 
TS1 <- TSeriesCT %>% filter(MetaNsp==nsp,DispersalDistance==alfa,Side==256,ColonizationRate==0.0001)  %>% mutate(MetaType=factor(MetaType,labels=c("Logseries"))) %>% group_by(MetaNsp,Side,MetaType,ColonizationRate,DispersalDistance,ReplacementRate,Rep) %>%
  mutate(
    isSpan = any(SpanningSpecies != 0),constSpan=length(unique(SpanningSpecies))>1) 
...
tClusters <- TS1 %>%filter( isSpan==TRUE | ReplacementRate<0.0002) %>% group_by(MetaNsp,Side,MetaType,ColonizationRate,DispersalDistance,ReplacementRate,Rep) %>% summarise(MaxClusterVar=var(MaxClusterProp),MaxClusterProp=mean(MaxClusterProp),n=n())

require(RColorBrewer)
colp <-brewer.pal(8,"Dark2")

ggplot(tClusters, aes(x=ReplacementRate, y=MaxClusterProp)) + geom_point() + theme_bw() + scale_x_log10() +   facet_grid(~ MetaType,scales="free_y" ) +xlab(bquote(rho)) + geom_line(colour=colp[1]) + geom_vline(aes(xintercept=pcrit),k,colour=colp[6]) + ylab("Mean of max cluster size")+  theme(axis.text.x=element_text(angle=-30, vjust=0.9,hjust = 0.1)) 
#ggsave("figs/SmaxvsRepl_T20000_320_256_meta_m.png", width=6,height=6,units="in",dpi=600)
...
# Calculate fluctuations.
# 
tClusters <- TS1  %>%filter(isSpan==TRUE | ReplacementRate<0.0002) %>%  group_by(MetaNsp,Side,MetaType,ColonizationRate,DispersalDistance,ReplacementRate,Rep) %>% mutate( Mean_MaxClusterProp=mean(MaxClusterProp), Delta_MaxCluster=MaxClusterProp-Mean_MaxClusterProp)
# mutate(MetaType=factor(MetaType,labels=c("Logseries","Uniform")))
...
#
vClusters <- tClusters %>% summarise(varDelta_MaxCluster=var(Delta_MaxCluster))
ggplot(vClusters, aes(x=ReplacementRate, y=varDelta_MaxCluster)) + geom_point(size=1) + theme_bw() + scale_x_log10(breaks=c(0.0000,0.0001,0.0002,0.0003,0.0005,0.0010, 0.01)) +   facet_grid(~ MetaType,scales="free_y" ) +xlab(bquote(rho)) + geom_line(colour=colp[1]) + geom_vline(aes(xintercept=pcrit),k,colour=colp[6]) + ylab(expression(Variance~of~Delta~S[max]))+  theme(axis.text.x=element_text(angle=-30, vjust=0.9,hjust = 0.1)) 
ggsave("figs/VarDeltaTSmaxvsRepl_T20000_320_256_meta_m.png", width=6,height=6,units="in",dpi=600)
