...
#
Models %>% group_by(MetaType,type,Side,ReplacementRate) %>% filter(DeltaAIC==0,type==1 | type==2,Side==512)  %>% summarise(n=n())
...
ggsave("figs/ClusBestModel_T20000_64_512_meta.png", width=6,height=6,units="in",dpi=600)


ggplot(mMdl, aes(x=ReplacementRate,y=Frequency,fill=model)) + geom_bar(stat="identity") +  scale_fill_manual(values=mc,name="",labels=c("Exp.","Power","Power Exp.")) + theme_bw() + theme(axis.text.x=element_text(angle=-30, vjust=0.9,hjust = 0.1)) + scale_x_continuous(breaks=c(0.0000,0.0001,0.0003,0.0005,0.0010, 0.01))  + xlab(bquote(rho))
...
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=model)) +  geom_point(size=0.5) + theme_bw() + facet_grid( ~MetaType ) +  stat_summary(fun.y=mean,geom="line") + scale_x_continuous(breaks=c(0.0000,0.0001,0.0002,0.0003,0.0005,0.0010, 0.01)) +
 theme(axis.text.x=element_text(angle=-30, vjust=0.9,hjust = 0.1)) + ylab(bquote(alpha)) + xlab(bquote(rho)) + scale_colour_manual(values=mc)+ coord_cartesian(ylim=c(1.5,4)) 
...
ggplot(mMdl,aes(x=ReplacementRate,y=alfa)) +  geom_point(size=0.5,colour=mc[2]) + theme_bw() + facet_grid( ~MetaType ) +  stat_summary(fun.y=mean,geom="line",colour=mc[2]) + scale_x_continuous(breaks=c(0.0000,0.0001,0.0002,0.0003,0.0005,0.0010, 0.01)) +
...
ggsave("figs/ExpAlfaVsRho_T20000_64_512_meta.png", width=7,height=4,units="in",dpi=600)
...
ggplot(mMdl,aes(x=ReplacementRate,y=rate)) +  geom_point(size=0.5,colour=mc[2]) + theme_bw() + facet_grid( ~MetaType )+  stat_summary(fun.y=mean,geom="line",colour=mc[2]) + scale_x_continuous(breaks=c(0.0000,0.0001,0.0002,0.0003,0.0005,0.0010, 0.01)) +
