...
#
group_by(Models,Side,model) %>% filter(DeltaAIC==0,ReplacementRate==0.01,type==1 | type ==2)  %>% summarise(n=n()) %>% ungroup() %>% mutate(Frequency=n/sum(n))
...
#
# No differences bteween Metacommunity or type (Spanning or max patch)
#
...
ggplot(mMdl,aes(x=ReplacementRate,y=alfa,colour=model)) +  geom_point(size=0.5) + theme_bw() +  stat_summary(fun.y=mean,geom="line") + scale_x_continuous(breaks=c(0.0000,0.0001,0.0003,0.0005,0.0010, 0.01)) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + ylab(bquote(alpha)) + xlab(bquote(rho)) + scale_colour_manual(values=mc)+ coord_cartesian(ylim=c(1.5,4)) 

ggsave("figs/AlfaVsRho_T20000_64_side_meta.png", width=7,height=4,units="in",dpi=600)
 
# Plot exponential decay rate
