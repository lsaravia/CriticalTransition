...
We calculated the sensitivity of critical point $\rho_c$ for infinite lattices varying three parameters one at a time: the migration $m$, the dispersal distance and the number of species on the metacommunity. To change the dispersal distance we vary the power exponent of the dispersal kernel $\alpha$. The range of parameters we used is in table 1, we used as a base the first row of the table, so if we vary $m$, the other parameters are fixed at $\alpha=2.04$ and the metacommunity number of species equal to 64. 
