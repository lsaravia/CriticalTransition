...
#
group_by(BCIMdl,model) %>% filter(DeltaAIC==0)  %>% summarise(n=n()) %>% ungroup() %>% mutate(Frequency=n/sum(n))
mMdl <- group_by(BCIMdl,model,type) %>% filter(DeltaAIC==0)  %>% summarise(n=n()) %>% ungroup() %>% mutate(Frequency=n/sum(n),type=namType[type])
mMdl

# Max Cluster Size proportion
ungroup(BCIMdl) %>% filter(type==2,DeltaAIC==0) %>% mutate(maxPatch = ClusterSize/(1000*500)) %>% summarise(maxPatchMean=mean(maxPatch))
...
All slopes are non-significative
